...
describe(`C51272: Dossier-Chronik: Einträge verwalten (delete); 
  ...
  'Speichern' and 'Loschen' ribbon buttons are visible but disabled"`, () => {
    pages.versicherte.detail.dossierChronikTab.dossierHistoryGrid.elements.gridWrapper().find("tr[class*='material']").its("length").then(size => {
      cy.log(size);
      for (let i = 0; i < size; i++) {
        pages.versicherte.detail.dossierChronikTab.dossierHistoryGrid.elements.gridWrapper().find("tr[class*='material']").first().xpath(".//td[1]").click();
        pages.versicherte.detail.ribbonMenu.clickLoschenBtn();
        pages.confirmPopup.clickJaBtn();
